...
export function Navbar({ logo, brand, items = [], align = 'spread', onItemClicked, children }: NavbarProps) {
  ...
  function toggleNavDropdown(item: NavItem, e: React.MouseEvent) {
    e.stopPropagation()
    if (openDropdownLabel === item.label) { setOpenDropdownLabel(null); return }
    ...
    <div className="navbar-host" ref={hostRef}>
      <nav className="navbar">
        <div className="navbar-container">
            ...
            {items.map(item =>
              item.children?.length ? (
                ...
                >
                  ...
                  <svg className={`navbar-chevron${openDropdownLabel === item.label ? ' rotated' : ''}`}
                    width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2 4l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </button>
              ) : item.route ? (
                <Link key={item.label} className="navbar-item" href={item.route} onClick={() => onNavItemClick(item)}>
                  {item.icon && <span className="navbar-item-icon">{item.icon}</span>}
                  <span className="navbar-item-label">{item.label}</span>
                  {item.badge != null && <span className="navbar-badge">{item.badge}</span>}
                </Link>
          ...
          <button
            ...
            onClick={() => { setIsMobileMenuOpen(v => !v); setOpenDropdownLabel(null) }}
      ...
      {items.map(item =>
        item.children?.length && openDropdownLabel === item.label ? (
          <div key={item.label} className="navbar-nav-dropdown" style={{ left: dropdownLeftPx }} role="menu">
            {item.children.map(child =>
              child.route ? (
                <Link key={child.label} className="navbar-nav-option" href={child.route}
                  onClick={() => { setOpenDropdownLabel(null); onNavItemClick(child) }} role="menuitem">
                  {child.icon && <span className="navbar-option-icon">{child.icon}</span>}
                  {child.label}
                </Link>
              ) : (
                <a key={child.label} className="navbar-nav-option" href={child.href || '#'}
                  onClick={(e) => { e.preventDefault(); setOpenDropdownLabel(null); onNavItemClick(child) }} role="menuitem">
                  {child.icon && <span className="navbar-option-icon">{child.icon}</span>}
                  {child.label}
                </a>
              )
          ...
          {items.map(item =>
            item.children?.length ? (
              <div key={item.label} className="navbar-mobile-group">
                ...
                {item.children.map(child =>
                  child.route ? (
                    <Link key={child.label} className="navbar-mobile-item navbar-mobile-child"
                      href={child.route} onClick={() => onNavItemClick(child)}>
              ...
              <Link key={item.label} className="navbar-mobile-item" href={item.route} onClick={() => onNavItemClick(item)}>
                {item.icon && <span className="navbar-item-icon">{item.icon}</span>}
                <span>{item.label}</span>
                {item.badge != null && <span className="navbar-badge">{item.badge}</span>}
              </Link>
            ) : (
              <a key={item.label} className="navbar-mobile-item" href={item.href || '#'}
                onClick={(e) => { e.preventDefault(); onNavItemClick(item) }}>
                {item.icon && <span className="navbar-item-icon">{item.icon}</span>}
                <span>{item.label}</span>
                {item.badge != null && <span className="navbar-badge">{item.badge}</span>}
