...
export function AutocompleteField({
  ...
  disabled = false, required = false, multi = false,
  value, values = [], onChange, onChanges,
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (wrapperRef.current && !wrapperRef.current.contains(e.target as Node)) {
        setIsOpen(false); setHighlighted(-1)
      }
    }
  ...
  function onKeydown(e: React.KeyboardEvent) {
    if (disabled) return
    if (e.key === 'ArrowDown') { e.preventDefault(); setHighlighted(i => Math.min(i + 1, filteredOptions.length - 1)) }
  ...
  return (
    <div className="field">
      ...
      >
        ...
        >
          {multi && selectedMulti.map(v => {
            const opt = options.find(o => o.value === v)
            return opt ? (
              <span key={v} className="chip">
                {opt.label}
                <button type="button" className="chip-remove" onClick={(e) => { e.stopPropagation(); removeChip(v) }} aria-label={`Remove ${opt.label}`}>×</button>
              </span>
            ) : null
          })}
          <input
            ...
            placeholder={multi ? (selectedMulti.length === 0 ? placeholder : '') : (selectedSingle === '' ? placeholder : singleLabel)}
            value={query}
            disabled={disabled}
        ...
        {isOpen && (
          <div className="field-dropdown" role="listbox">
            {filteredOptions.length > 0 ? filteredOptions.map((option, i) => (
              ...
              <div className="field-option no-results" style={{ opacity: 0.6, cursor: 'default' }}>No results found</div>
            )}
          </div>
        )}
      </div>
