...
interface FlatItem { item: DropdownItem; gi: number; i: number }
...
export function Dropdown({ items = [], groups = [], placement = 'bottom-start', onSelected, children }: DropdownProps) {
  ...
  function key(gi: number, i: number) { return `${gi}:${i}` }
  function isSubmenuOpen(gi: number, i: number) { return openSubmenuKey === key(gi, i) }
  function isFocused(gi: number, i: number) {
    const f = flatItems[focusedIdx]
    return f ? f.gi === gi && f.i === i : false
  }

  function close() { setIsOpen(false); setOpenSubmenuKey(''); setFocusedIdx(-1) }

  function emitItem(item: DropdownItem) { onSelected?.(item); close() }
  ...
  function onTriggerClick(e: React.MouseEvent) {
    e.stopPropagation()
    if (isOpen) close()
    else { setIsOpen(true); setFocusedIdx(-1) }
  }

  function onTriggerKeydown(e: React.KeyboardEvent) {
    if (e.key === 'Enter' || e.key === ' ' || e.key === 'ArrowDown') {
      e.preventDefault(); setIsOpen(true); setFocusedIdx(0)
    }
  }
  ...
  function onPanelKeydown(e: React.KeyboardEvent) {
    ...
    switch (e.key) {
      case 'ArrowDown': e.preventDefault(); setFocusedIdx(i => Math.min(i + 1, flatItems.length - 1)); setOpenSubmenuKey(''); break
      case 'ArrowUp': e.preventDefault(); setFocusedIdx(i => Math.max(i - 1, 0)); setOpenSubmenuKey(''); break
      case 'ArrowRight': e.preventDefault(); if (idx >= 0 && flatItems[idx]?.item.children?.length) setOpenSubmenuKey(key(flatItems[idx].gi, flatItems[idx].i)); break
      case 'ArrowLeft': e.preventDefault(); setOpenSubmenuKey(''); break
      case 'Enter': {
        e.preventDefault()
      ...
      case 'Escape': e.preventDefault(); if (openSubmenuKey) setOpenSubmenuKey(''); else close(); break
  ...
  return (
    <div ref={wrapperRef} className="dropdown-host" onClick={(e) => e.stopPropagation()}>
      ...
      {isOpen && (
        <div className={`dropdown-panel${placement === 'bottom-end' ? ' align-end' : ''}`} role="menu" onKeyDown={onPanelKeydown} tabIndex={-1}>
                ...
                return (
                  <div key={i} className="dropdown-item-wrapper" style={{ position: 'relative' }}>
                    <div
                      ...
                      onClick={(e) => { e.stopPropagation(); if (!item.disabled && !item.separator) { if (hasChildren) return; emitItem(item) } }}
                      onMouseEnter={() => {
                    ...
                    >
                      ...
                      {hasChildren && (
                        <svg className="item-arrow" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"
                        ...
                        {item.children!.map((child, ci) => (
                          child.separator ? (
                            ...
                            <div
                              ...
                              onClick={(e) => { e.stopPropagation(); if (!child.disabled) emitItem(child) }}
                            >
