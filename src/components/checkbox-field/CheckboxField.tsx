...
export function CheckboxField({
  label, hint, validateMessage, checked = false, disabled = false,
  required = false, indeterminate = false, onChange
}: CheckboxFieldProps) {
  ...
  return (
    <div className="field checkbox-field">
      <label className={`checkbox-label${disabled ? ' disabled' : ''}`} onClick={toggle}>
        <div className={`checkbox-box${isChecked || indeterminate ? ' checked' : ''}${disabled ? ' disabled' : ''}${validateMessage ? ' error' : ''}`}>
