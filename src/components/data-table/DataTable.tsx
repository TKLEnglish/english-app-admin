...
export function DataTable({
  columns = [], data = [], loading = false, filterable = false,
  filterPlaceholder = 'Filter rows…', emptyMessage = 'No data available',
      ...
      {filterable && (
        <div className="data-table-filter">
          <div className="data-table-filter-wrap">
            <svg className="data-table-filter-icon" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
            </svg>
            <input type="text" className="data-table-filter-input" placeholder={filterPlaceholder}
              value={filterTerm} onChange={onFilter} />
          </div>
        </div>
      )}
            ...
            <tr>
              {columns.map(col => (
                <th
                  ...
                  onClick={() => onSort(col)}
                  aria-sort={sortState.key === col.key ? (sortState.direction === 'asc' ? 'ascending' : sortState.direction === 'desc' ? 'descending' : 'none') : undefined}
                >
                  <span>{col.label}</span>
                  {col.sortable && (() => {
            ...
            {loading ? (
              Array.from({ length: SKELETON_ROWS }).map((_, ri) => (
                <tr key={ri} className="dt-skeleton-row">
                  {columns.map((_, ci) => (
                    <td key={ci}><div className="dt-skeleton-cell" /></td>
                  ))}
                </tr>
              ))
            ) : sortedData.length === 0 ? (
              ...
              sortedData.map((row, ri) => (
                <tr key={ri}>
                  {columns.map(col => {
                    ...
                    const content: ReactNode = React.isValidElement(val) ? val : (val != null ? String(val) : '')
                    return (
                      <td key={col.key} className="dt-td">{content}</td>
                    )
                  })}
                </tr>
              ))
      ...
      {onPageChange && totalPages != null && totalPages > 1 && page != null && (
        <div className="dt-pagination">
          ...
          <div className="dt-pagination-controls">
            <button className="dt-page-btn" disabled={page <= 1} onClick={() => onPageChange(1)} aria-label="First page">«</button>
            <button className="dt-page-btn" disabled={page <= 1} onClick={() => onPageChange(page - 1)} aria-label="Previous page">‹</button>
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((p, i) =>
                p === 'ellipsis'
                  ...
                  : <button key={p} className={`dt-page-btn${p === page ? ' active' : ''}`} onClick={() => onPageChange(p as number)}>{p}</button>
              )}
            <button className="dt-page-btn" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} aria-label="Next page">›</button>
            <button className="dt-page-btn" disabled={page >= totalPages} onClick={() => onPageChange(totalPages)} aria-label="Last page">»</button>
          </div>
        </div>
      )}
    </div>
